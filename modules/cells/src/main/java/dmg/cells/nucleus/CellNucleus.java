...
public class CellNucleus implements ThreadFactory {
    ...
    private enum State {
        ...
        /**
         * Whether it is legal for a cell to call {@link #sendMessage(CellMessage, boolean, boolean,
         * boolean, CellMessageAnswerable, Executor, long)}.
         */
        boolean isSendWithCallbackAllowed;

        /**
         * Whether callbacks are guaranteed to be called. At some point during shutdown, the timeout
         * mechanism is stopped and callbacks are no longer called automatically.
         */
        boolean areCallbacksGuaranteed;

        State(int externalState, boolean isStarting, boolean isSendWithCallbackAllowed,
              boolean areCallbacksGuaranteed) {
    ...
    /**
     * Timer for periodic low-priority maintenance tasks. Shared among all cell instances. Since a
     * Timer is single-threaded, it is important that the timer is not used for long-running or
    ...
    private static final ScheduledExecutorService _timer = Executors.newSingleThreadScheduledExecutor(
          new ThreadFactoryBuilder().setDaemon(true).setNameFormat("Cell maintenance task timer")
                .build());
    ...
    public CellNucleus(Cell cell, String name, String type, Executor executor) {
        ...
        _messageExecutor = (executor == null) ? new BoundedCachedExecutor(this, 1)
    ...
    public void sendMessage(CellMessage msg, boolean locally, boolean remotely,
        ...
        checkArgument(!msg.isFinalDestination(), "Message has no next destination: %s",
    ...
    /**
     * Sends <code>envelope</code> and waits <code>timeout</code> milliseconds for an answer to
     * arrive.  The answer will bypass the ordinary queuing mechanism and will be delivered before
     * any other asynchronous message.  The answer need to have the getLastUOID set to the UOID of
     * the message send with sendAndWait. If the answer does not arrive withing the specified time
     * interval, the method returns <code>null</code> and the answer will be handled as if it was an
     * ordinary asynchronous message.
     * <p>
     * This method mostly exists for backwards compatibility. dCache code should use CellStub or
     * CellEndpoint.
     *
     ...
     * @throws NoRouteToCellException if the destination could not be reached.
    ...
    private void executeMaintenanceTasks() {
        ...
        Iterables.limit(consumingIterable(_deferredTasks), _deferredTasks.size())
     ...
     * Sends <code>msg</code>.
     * <p>
     * The <code>callback</code> argument specifies an object which is informed as soon as an has
     * answer arrived or if the timeout has expired.
     * <p>
     * The callback is run in the supplied executor. The executor may execute the callback inline,
     * but such an executor must only be used if the callback is non-blocking, and the callback
     * should refrain from CPU heavy operations. Care should be taken that the executor isn't
     * blocked by tasks waiting for the callback; such tasks could lead to a deadlock.
     *
     ...
     * @param callback        specifies an object class which will be informed as soon as the
     *                        message arrives.
    ...
    public void sendMessage(CellMessage msg,
        ...
        checkState(_state.isSendWithCallbackAllowed,
              "Cannot send message with callback in state {}", _state);
        checkArgument(!msg.isFinalDestination(), "Message has no next destination: %s",
              msg.getDestinationPath());
    ...
    /**
     * The kill method schedules the specified cell for deletion. The actual remove operation will
     * run in a different thread. So on return of this method the cell may or may not be alive.
     */
    ...
    /**
     * Log the threads of some cell.  This is intended for diagnostic information.
     */
    ...
    /**
     * Log the killer threads.  This is intended for diagnostic information.
     */
    ...
    /**
     * Waits for at most timeout milliseconds for the termination of a set of threads.
     *
     ...
     * <p>
     * Calls the startup callbacks of the cell, registers the cell with the cell glue and initiates
     * cell message delivery. If startup fails, the cell is torn down.
     * <p>
    ...
    private void doStart() {
        try {
            ...
            _timeoutTask = _timer.scheduleWithFixedDelay(
                  wrapLoggingContext(this::executeMaintenanceTasks),
    ...
    private void rescheduleCallbackTimeout() {
        ...
        if (earliestExpiringCallback != null) {
            long delayUntilFirstTimeout =
                  earliestExpiringCallback.getTimeout() - System.currentTimeMillis();

            if (_scheduledTimeoutRun != null) {
                long delayUntilNextTimeoutRun = _scheduledTimeoutRun.getDelay(
            ...
            _scheduledTimeoutRun = _timer.schedule(
                  new FireAndForgetTask(this::processTimedOutMessages),
    ...
    /**
     * Unregisters the callback and calls its timeout method. If scheduling the callback fails that
     * task is reregistered for later processing.
     */
     ...
     * @throws IllegalArgumentException If the route is a duplicate or if it routes through a
     *                                  non-existing local cell.
     */
